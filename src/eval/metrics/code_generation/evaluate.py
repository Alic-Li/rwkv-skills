...
It then writes canonical evaluator output (results/eval) with only:
benchmark_name, dataset_split, sample_index, repeat_index, context, is_passed, fail_reason
"""
...
def _write_canonical_eval_from_results(results_path: Path, *, eval_output_path: Path) -> None:
...
def evaluate_human_eval(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="rwkv_skills_humaneval_") as tmp_dir:
        ...
        _write_canonical_eval_from_results(Path(results_path_str), eval_output_path=eval_output_path)
        return metrics or {}
